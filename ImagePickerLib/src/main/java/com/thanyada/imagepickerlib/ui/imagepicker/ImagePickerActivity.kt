...
import com.thanyada.imagepickerlib.databinding.ImagepickerActivityImagepickerBinding
import com.thanyada.imagepickerlib.helper.*
...
import com.thanyada.imagepickerlib.ui.camera.CameraModule
import com.thanyada.imagepickerlib.ui.camera.OnImageReadyListener
...
class ImagePickerActivity : AppCompatActivity(), OnFolderClickListener, OnImageSelectListener {
            ...
            object : PermissionHelper.PermissionAskListener {
                ...
                override fun onPermissionDisabled() {
                    binding.snackbar.show(
                        R.string.image_picker_msg_no_external_storage_permission
                    ) {
    ...
    private fun onCropImage(selectedImage: Image) {
        config.cropOptions?.let {
            cropImage.launch(
                options(uri = selectedImage.uri, it)
            )
        }
    }
    ...
    override fun onSingleModeImageSelected(image: Image) {
        if(config.cropOptions != null){
            onCropImage(image)
        }else finishPickImages(ImageHelper.singleListFromImage(image))
    }
}